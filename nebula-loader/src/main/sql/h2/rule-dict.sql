...
(
    ...
    `source`            INT(11)           NOT NULL DEFAULT '-1' COMMENT '来源: 1-搜索,2-信息流,4-APP, 8-视频',
    `operator_id`       BIGINT        NOT NULL DEFAULT '-1' COMMENT '审核员id',
    `word`              VARCHAR(1024) NOT NULL DEFAULT '' COMMENT '关键词',
    `word_type`         INT(11)           NOT NULL COMMENT '词表类型, 1:黑名单，2：商标词，3：竞品词，4：敏感词',
    `match_type`        INT(11)           NOT NULL DEFAULT '-1' COMMENT '匹配规则, 0:分散,1:包含,2:精确',
    `entity_type`       INT(11)           NOT NULL DEFAULT '-1' COMMENT '实体类型,支持按位与运算,1:创意,2:关键词',
    ...
    `status`            INT(11)       NOT NULL DEFAULT '1' COMMENT '规则状态, 0:待评估, 1:生效, 2:失效',
...
(
    ...
    `source`        INT(11)       NOT NULL DEFAULT 0 COMMENT '来源: 1-搜索,2-信息流,4-APP, 8-视频',
